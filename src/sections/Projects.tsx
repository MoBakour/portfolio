...
const ProjectCard = ({ project }: IProjectCard) => {
    return (
        <a href={project.repo} target="_blank">
            <div className="group-hover/container:opacity-60 max-md:opacity-100! hover:opacity-100! group/card p-4 py-6 flex max-sm:flex-col-reverse gap-4 transition rounded-lg hover:bg-card-bg/50 hover:shadow-[inset_0_1px_0_0_rgba(148,163,184,0.1)] hover:drop-shadow-lg max-md:hover:bg-transparent max-md:hover:shadow-none max-md:hover:drop-shadow-none cursor-pointer">
                <div className="flex flex-col w-fit gap-2">
                    ...
                    <div className="flex justify-center items-center gap-2">
                        <a
                            href={project.repo}
                            className="text-2xl m-2 transition hover:text-text"
                            title="Github repo"
                            target="_blank"
                        >
                            <FaGithub />
                ...
                <div className="flex flex-col gap-2">
                    <h3 className="text-text font-bold text-lg group-hover/card:text-primary max-md:group-hover/card:text-text transition">
                        {project.title}
                        <FiExternalLink className="inline-block ml-2 mb-2" />
                    </h3>
                    <p className="text-sm">{project.description}</p>
                    <Bullets bullets={project.stack as unknown as string[]} />
                </div>
            </div>
        </a>
    );
...
const Projects = () => {
    ...
    return (
        <section>
            ...
            {showIncrement && (
                <button
                    className="hover:text-primary w-fit m-auto flex flex-col justify-center items-center transition cursor-pointer"
